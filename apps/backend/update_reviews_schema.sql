-- 1. quest_idカラムを追加（デフォルト値12）
ALTER TABLE reviews ADD COLUMN quest_id INT NOT NULL DEFAULT 12;

-- 2. 外部キー制約を追加
ALTER TABLE reviews ADD CONSTRAINT reviews_quest_id_fkey
FOREIGN KEY (quest_id) REFERENCES quests(id);

-- 3. 既存データのquest_idを適切な値に更新
-- シードデータに基づいて、レビューIDに対応するクエストIDを設定
...
-- 4. 一意性制約を追加（reviewer_id, quest_idの組み合わせ）
