...
describe("questService", () => {
  ...
  describe("getAllQuests", () => {
    ...
    it("フィルタパラメータを渡して取得する", async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith("/quests", { status: "active", keyword: "test" });
    });
  });
  ...
  describe("deleteQuest", () => {
    it("認証付きでクエストを削除する", async () => {
      vi.mocked(authenticatedApiClient.delete).mockResolvedValueOnce({ message: "deleted" });
  ...
  describe("getAllQuestsIncludingDeleted", () => {
    it("管理者用エンドポイントを認証付きで呼ぶ", async () => {
      ...
      expect(authenticatedApiClient.get).toHaveBeenCalledWith("/quests/admin/all", undefined);
    });
  });
});
